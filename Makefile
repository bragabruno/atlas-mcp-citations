# atlas-mcp-citations — build system (single source of truth).
...
test: ## Offline unit tests (pytest; fake ES + Qdrant backends)
	@./scripts/test.sh

coverage: ## Coverage gate (pytest-cov; recommended, ATLAS_COV_MIN)
	@./scripts/coverage.sh

build: ## Build verification — MCP server import smoke (app.server)
	@./scripts/build.sh
