# atlas-mcp-citations — build scripts
...
The diagram-validation gate (XCUT-6) keeps its own entrypoint,
`scripts/validate_diagrams.sh` (Mermaid + PlantUML), invoked directly by the
pipeline.
